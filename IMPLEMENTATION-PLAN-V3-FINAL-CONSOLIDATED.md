# Vista Website V3 - Implementation Plan (Final Consolidated)
**Date:** 2026-02-16  
**Status:** Awaiting Isaac approval  
**Scope:** Complete redesign with all feedback consolidated
...
**Core principles:**
1. **Square everything** — Border-radius 0-2px max across entire site
...
- **Vista landing2 (main branch):** Hero decorative symbols, newsletter design pattern
- V1 Vista: Sacred brand elements (stars ₊˚⊹, color #FF5233)
...
- **Headline:** "**vista** investigates the future of the internet" (lowercase, "vista" has orange underline decoration)
  - **Underline:** ⚠️ **4px solid #FF5233** (increased from 2px), offset 0.18em below text
...
**Decorative elements (from landing2 main branch):**
- **Use exact pattern from `/src/components/Hero.tsx` (main branch)**
- Implementation:
  ```tsx
  ...
  ```
- **Icons source:** `/src/components/icons.tsx` (RingIcon, GridIcon, PlusIcon)
- **Color:** #FF5233, rotation **0deg ALWAYS** (sacred element)
- **Parallax:** Use transform translateZ for depth effect
...
- **Mobile:** ⚠️ **Single column stack, ALL cards 1:1 aspect (including "latest" card)**
  - Large "latest" card: **same size as others** (not bigger)
  - **NO "latest" badge** (not shown on any viewport, position implies it)
...
- **Content area:**
  ...
  - Title: 22px weight 500 (grid cards) / 32px (large card desktop only)
  - Excerpt: 18px muted (2 lines clamp)
  - "READ MORE →" link: 16px, color orange on hover
  
- **Badge on large card:**
  - ⚠️ **NO BADGE** — Remove "latest" badge entirely (not shown on any viewport)
  - Large card is implied "latest" by position alone
...
**CRITICAL:** Use design pattern from `/src/components/Newsletter.tsx` (main branch) as foundation, adapted to full-screen.

**Section specs:**
- **Height:** 100vh (full-screen, NOT a card)
...
**Content layout (adapted from main branch Newsletter.tsx):**
```
┌─────────────────────────────────────────┐
...
**Styling (from main branch, adapted):**
- **Use CSS classes pattern from Newsletter.tsx:** `newsletter-hero-*` naming convention
- Headline: h1 48px, white, center-aligned on mobile, left on desktop
...
- Social proof chip: "10k+ subscribers" badge (same style as main branch)

**Background decoration:**
- **Use `/public/frame-110.svg`** (orange curved lines, from main branch)
- Position: `bg-[position:170%_center]` mobile, `bg-[position:100%_center]` desktop
- Size: `bg-[length:auto_125%]` mobile, `bg-[length:auto_108%]` desktop
- Opacity: Adjust via overlay div if needed (0.15-0.2)
- **Reference exact implementation from main branch Newsletter.tsx**

**Key differences from card version:**
- Full viewport height (100vh) instead of card height
- Dark background (#111111) instead of light (#f7f7f7)
- Centered vertically in viewport
- Triggers navbar theme switch
...
### Hero Decorative Symbols (from main branch)
- **File to reference:** `/src/components/Hero.tsx` (main branch)
- **Icons file:** `/src/components/icons.tsx` (RingIcon, GridIcon, PlusIcon)
- Use exact parallax-container pattern with perspective: 1000px
- Transform translateZ for depth layers (-200px, -150px, -100px)
- Responsive sizing with clamp()
- Color: #FF5233 (text-ultra-orange)
- **NO rotation** (0deg always, sacred element)

### Newsletter Design (from main branch)
- **File to reference:** `/src/components/Newsletter.tsx` (main branch)
- Adapt card design to full-screen viewport
- Keep CSS class naming convention: `newsletter-hero-*`, `newsletter-proof-*`, etc.
- Preserve layout structure (kicker + title + bullets + form + partners)
- Use same background SVG pattern (`/public/frame-110.svg`)
- Maintain responsive breakpoints from original
...
- Decorative SVGs from main branch Hero.tsx preserved exactly

### Mobile Considerations
- Latest card: 1:1 aspect, same size as other cards (no badge on any viewport)
- Newsletter: stack email input + button vertically on mobile
- Grid lines: reduce opacity or hide on <640px if too busy
- Touch targets: minimum 44×44px
- Hero decorative symbols: Hide or reduce on mobile (responsive via sm:block classes)

---

## ⚠️ CRITICAL CHANGES FROM PREVIOUS PLAN

1. **Hero underline:** Increased from 2px → **4px** (thicker, more visible)
2. **Latest card:** 
   - Mobile: **Same 1:1 aspect as other cards** (not larger)
   - **NO "latest" badge** on any viewport (desktop or mobile) — position implies it
3. **Hero decorative elements:** Use exact pattern from `/src/components/Hero.tsx` (main branch)
4. **Newsletter design:** Base on `/src/components/Newsletter.tsx` (main branch), adapted to full-screen
...
- [x] Section specs detailed with main branch references
- [x] Technical notes included
- [x] Critical changes highlighted
- [ ] **Isaac approval** ← WAITING

**Designer subagent will:**
1. Create detailed component specs (buttons, cards, inputs)
2. Reference main branch code (Hero.tsx, Newsletter.tsx) for exact patterns
3. Define exact spacing/sizing for each section
4. Specify animations and interactions
5. Validate against references (Chaos Labs, Stripe, main branch Vista)
6. Deliver: `DESIGN-SPEC-V3-REVISED-FINAL.md`

**Developer subagent (Codex) will:**
1. **DELETE** all `/src/app/landing2/` and `/src/components/landing2/` code FIRST
2. **Reference main branch files:**
   - `/src/components/Hero.tsx` for decorative symbols pattern
   - `/src/components/Newsletter.tsx` for newsletter design
   - `/src/components/icons.tsx` for icon components
3. Implement from scratch based on approved Design Spec
4. Sacred elements checks (stars, colors, rotation 0deg)
5. RSS integration + fallback gradients
6. Navbar theme switcher with IntersectionObserver
7. Grid lines system
8. Mobile: 1:1 cards, no "latest" badge
9. Hero underline: 4px thickness
10. Test: `npm run build`, localhost preview, ngrok preview
11. **QA verification before reporting:**
    - Curl localhost to verify content
    - Curl ngrok to verify routing
    - Screenshot both URLs
12. Deliver: Working code + screenshots + completion report

**Validation (Jarvis):**
1. Spot-check for recycled code (must be 0%)
2. Sacred elements preserved exactly (stars 0deg, #FF5233)
3. Square philosophy applied globally (border-radius 0-2px)
...
8. Hero underline 4px (not 2px)
9. Latest card 1:1 (mobile), no badge anywhere (desktop or mobile)
10. Decorative symbols match main branch pattern
...
3. Jarvis spawns Designer (Opus 4.6) with main branch references
4. Designer delivers revised DESIGN-SPEC, Jarvis validates
5. Isaac approves spec
6. Jarvis spawns Developer (Codex) with delete-first + reference-main-branch mandate
7. Developer implements, reports to Jarvis with QA verification
8. Jarvis validates implementation (10-point checklist)
9. Show to Isaac for final review
10. Deploy to vista.wtf (separate task)
...
- Development (full implementation with QA): 90-120 min
