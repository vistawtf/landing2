...
function ArticleCard({ article, isLarge = false }: { article: Article; isLarge?: boolean }) {
  return (
    <Link
      href={article.link}
      className={`group block bg-transparent overflow-hidden transition-colors duration-200 h-full ${isLarge ? '' : 'aspect-square'}`}
    >
      <div
        className={`w-full relative ${isLarge ? 'h-[48%] md:h-[52%]' : 'h-[42%]'} border-b border-black/[0.08]`}
        style={{
      ...
      <div className="p-4 md:p-5">
        ...
        <h3 className={`${isLarge ? 'text-[28px] md:text-[34px]' : 'text-[18px] md:text-[21px]'} font-medium text-[#111] mb-3 leading-[1.08] line-clamp-2`}>
          {article.title}
        </h3>

        <p className={`text-[#575757] mb-5 ${isLarge ? 'text-[17px] md:text-[18px] leading-[1.45] line-clamp-3' : 'text-[14px] leading-[1.5] line-clamp-2'}`}>
          {article.excerpt}
        </p>

        <div className="text-[13px] font-medium uppercase tracking-[0.05em] text-[#575757] group-hover:text-[#FF5233] transition-colors">
  ...
  return (
    <section id="latest" className="relative py-20 md:py-32 bg-[#E4E2D8]">
      ...
      <div className="relative z-10 max-w-[1200px] mx-auto px-6 md:px-16">
        ...
        <div className="hidden md:grid md:grid-cols-[1.2fr_1fr] gap-0 border border-black/[0.10]">
          {latest && (
