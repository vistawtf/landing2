...
// Based on: NYT, a16z pattern for B2B thought leadership
// Optimizes for: Readability + Authority positioning
  ...
  return (
    ...
    >
      <article className="h-full flex flex-col">
        {/* Large card: Keep image */}
        {isLarge && (
          <div
            className="w-full h-[50%] relative border-b border-white/[0.08]"
            style={{
        ...
        <div className={`${isLarge ? 'p-5 md:p-6' : 'p-5'} flex-1 flex flex-col justify-between min-h-0`}>
          {/* Category badge */}
          <div>
            <div className="text-[11px] font-semibold uppercase text-[#999999] tracking-[0.12em] mb-3">
              {article.category}
            </div>

            {/* Title - bigger for small cards */}
            <h3 className={`${isLarge ? 'text-[28px] md:text-[32px]' : 'text-[22px] md:text-[24px]'} font-medium text-white mb-3 leading-[1.15] line-clamp-3`}>
              {article.title}
            </h3>

            {/* Description - only on large card */}
            {isLarge && (
              <p className="text-[#cccccc] text-[16px] leading-[1.4] line-clamp-2">
                {article.excerpt}
              </p>
            )}
          </div>
          ...
          {/* Read more */}
          <div className="mt-auto pt-4 text-[13px] font-medium uppercase tracking-[0.05em] text-[#999999] group-hover:text-[#FF5233] transition-colors">
